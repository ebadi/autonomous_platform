...
Serial.println("max=" + String(motor_max_limit));
Serial.println("min=" + String(motor_min_limit));  
...
String KangarooControllInterface::SendSimplifiedSerialCommandWaitForResponse(String inputCmd) {
  ...
  while (waitloop) {
    ...
    //Serial.println("start t=" + String(startTime) + ", end t=" + String(endTime) + " end-start=" + String(endTime - startTime));
...
bool KangarooControllInterface::PerformKangarooStartupProcedure() {
  ...
  SetMotorAbsolutePosition(0); 
  
  delay(5000); //wait for motor to reach its position
  int pos = ReadCurrentMotorEncoderPosition();
  
  SendInformationDebugSerial("Pos is:" + String(pos) + ", error?=" + String(last_error));
