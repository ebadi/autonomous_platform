...
KangarooControllInterface::KangarooControllInterface() {
  port_nr = -1;
  motor_nr = -1;
  motorFixedSpeed = -1;
  wait_for_response_time = -1;
  last_error = "object initialized with empty constructor and values have not been set";
...
//explicit init func
//init func
bool KangarooControllInterface::init(Stream &inputCmdPort, int inputPortNr, int inputMotorNr, int inputMotorFixedSpeed, int inputWaitForResponseTime) {
  command_serial_port = &inputCmdPort;
  port_nr = inputPortNr;
  motor_nr = inputMotorNr;
  motorFixedSpeed = inputMotorFixedSpeed;
  ...
  bool finnished_procedure = PerformKangarooStartupProcedure();
  
  //setup was successful and motor encoder positions have been set
  if( finnished_procedure && motor_max_limit > motor_min_limit) {
    return true;
  }
  else {
    return false;
...
bool KangarooControllInterface::init(Stream &inputCmdPort, int inputPortNr, int inputMotorNr, int inputMotorFixedSpeed, int inputWaitForResponseTime, Stream &inputDebugPort) {
  command_serial_port = &inputCmdPort;  //&Serial1; works
  port_nr = inputPortNr;
  motor_nr = inputMotorNr;
  motorFixedSpeed = inputMotorFixedSpeed;
...
bool KangarooControllInterface::PerformKangarooStartupProcedure() {
  ...
  SendInformationDebugSerial("Homing steering motor! Waiting 5 seconds.");
  delay(5000);

  motor_max_limit = ReadMotorEncoderMaxLimit();
  motor_min_limit = ReadMotorEncoderMinLimit();
  int middlepos = (int) (motor_max_limit + motor_min_limit)/2;
  if(motor_max_limit == -32768 || motor_min_limit == -32768)
  {
    SendInformationDebugSerial("ERROR STARTUP PROCEDURE: could not read motor position max / min limit");
    SendInformationDebugSerial("Min limit=" + String(motor_min_limit) + ", max limit=" + String(motor_max_limit));
    return false;
  }
  
  SendInformationDebugSerial("Found limits: Min limit=" + String(motor_min_limit) + ", max limit=" + String(motor_max_limit));
  SendInformationDebugSerial("Moving to middle position = " + String(middlepos));
  SetMotorAbsolutePosition(middlepos);
