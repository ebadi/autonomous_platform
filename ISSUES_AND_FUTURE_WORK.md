...
Important note added: Once embedded sensors have been implemented according to `HOW_TO_EXTEND.md` information will be available on topic /GET_<FRAME ID>_<SIGNAL_NAME>. This information will then need to be transferred to known standard name topics to make the information flow compatible with the information sent from the digital twin.
