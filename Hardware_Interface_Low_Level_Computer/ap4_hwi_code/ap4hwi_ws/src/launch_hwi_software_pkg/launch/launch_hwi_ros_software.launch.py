...
def generate_launch_description():
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource([get_package_share_directory('ros2_socketcan'), '/launch/socket_can_receiver.launch.py']),
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource([get_package_share_directory('ros2_socketcan'), '/launch/socket_can_sender.launch.py']),
                       launch_arguments={'interface': 'can0', 'timeout_sec': '0.01'}.items()
        ),
        # launch custom made can translator node
        Node(
            package='can_msgs_to_ros2_topic_pkg',
            executable='can_ros2_interface'
        ),
        # start feedforward controller and xbox joystick input nodes
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource([get_package_share_directory('xbox_controller_feed_forward_ctrl_pkg'), '/launch/launch_xbox_controller_feedforward_ctrl.launch.py'])
        )
    ])
