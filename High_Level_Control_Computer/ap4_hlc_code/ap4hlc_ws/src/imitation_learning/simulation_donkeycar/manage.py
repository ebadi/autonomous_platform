...
def drive(
    ...
    V.add(
        aiLauncher, inputs=["user/mode", "pilot/throttle"], outputs=["pilot/throttle"]
    )
...
def add_user_controller(V, cfg, use_joystick, input_image="ui/image_array"):
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        #
        if (
            cfg.CONTROLLER_TYPE == "pigpio_rc"
        ):  # an RC controllers read by GPIO pins. They typically don't have buttons
            ...
            if (
                ...
                ctr = MockController(
                    steering=cfg.MOCK_JOYSTICK_STEERING,
...
def get_camera(cfg):
    ...
    if not cfg.DONKEY_GYM:
        if cfg.CAMERA_TYPE == "PICAM":
            ...
            cam = LICamera(
                width=cfg.IMAGE_W, height=cfg.IMAGE_H, fps=cfg.CAMERA_FRAMERATE
            )
...
def add_odometry(V, cfg, threaded=True):
    ...
    if cfg.HAVE_ODOM:
        ...
        kinematics = (
            UnicyclePose(cfg, poll_delay_secs)
            if cfg.HAVE_ODOM_2
            else BicyclePose(cfg, poll_delay_secs)
        )
        V.add(
            kinematics,
