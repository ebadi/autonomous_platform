...
CAMERA_HFLIP = False
CAMERA_INDEX = (
    0  # used for 'WEBCAM' and 'CVCAM' when there is more than one camera connected
)
...
SHOW_PILOT_IMAGE = (
    False  # show the image used to do the inference when in autopilot mode
)
...
VESC_BAUDRATE = (
    115200  # baudrate for the serial communication. Shouldn't need to change this.
)
...
HAVE_TFMINI = False
TFMINI_SERIAL_PORT = (
    "/dev/serial0"  # tfmini serial port, can be wired up or use usb/serial adapter
)
...
CANNY_LOW_THRESHOLD = (
    60  # Canny edge detection low threshold value of intensity gradient
)
CANNY_HIGH_THRESHOLD = (
    110  # Canny edge detection high threshold value of intensity gradient
)
CANNY_APERTURE = (
    3  # Canny edge detect aperture in pixels, must be odd; choices=[3, 5, 7]
)
...
JOYSTICK_DEADZONE = (
    0.01  # when non zero, this is the smallest throttle before recording triggered.
)
...
IMU_ADDRESS = (
    0x68  # if AD0 pin is pulled high them address is 0x69, otherwise it is 0x68
)
...
PIGPIO_STEERING_MID = (
    1500  # Adjust this value if your car cannot run in a straight line
)
...
DONKEY_GYM = True
DONKEY_SIM_PATH = os.path.expanduser(
    "~/projects/DonkeySimLinux/donkey_sim.x86_64"
...
GYM_CONF["racer_name"] = "Johan och Parvlid"
GYM_CONF["country"] = "Place"
GYM_CONF["bio"] = "I race robots."
GYM_CONF["cam_resolution"] = (IMAGE_W, IMAGE_H, IMAGE_DEPTH)
GYM_CONF["steer_limit"] = 0.5
GYM_CONF["throttle_min"] = -0.5
GYM_CONF["throttle_max"] = 0.5
...
SHOW_FPS = False
FPS_DEBUG_INTERVAL = (
    10  # the interval in seconds for printing the frequency info into the shell
)
